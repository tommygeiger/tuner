  ...
  function updateOptions(event, value, reason) {
    ...
    if (value && value!=='') {
      ...
      fetch(`https://api.spotify.com/v1/search?q=${value.split(' ').join('%20')}&type=track&limit=5`, { headers })
        ...
        .then(
          (result) => {
            console.log(result.tracks.items.map(track => ({ext_url:track.artists[0].external_urls.spotify,disc_num:track.disc_number, title:track.name, artist:track.artists[0].name, explicit:track.explicit})))
